...
export async function githubReadFile(owner: string, repo: string, path: string, ref: string | undefined) {
  const headers = githubHeaders();
...
export async function githubListDirectory(owner: string, repo: string, path: string, ref: string | undefined) {
  const headers = githubHeaders();
  ...
  const payload = (await fetchJson(url.toString(), { headers })) as Array<{
    name?: string;
    path?: string;
    type?: string;
    size?: number;
...
export async function cratesPackageInfo(crateName: string) {
  return fetchJson(`https://crates.io/api/v1/crates/${encodeURIComponent(crateName)}`, {
    headers: {
      "User-Agent": "hva-pi-tools",
    },
  });
...
export async function cratesDependencies(crateName: string, version: string | undefined) {
  ...
  return fetchJson(`https://crates.io/api/v1/crates/${encodeURIComponent(crateName)}/${encodeURIComponent(resolvedVersion)}/dependencies`, {
    headers: {
      "User-Agent": "hva-pi-tools",
    },
  });
...
export async function docsRsRead(crateName: string, version: string | undefined, path: string | undefined) {
  const url = new URL(`https://docs.rs/${encodeURIComponent(crateName)}/${version ? encodeURIComponent(version) : "latest"}/${path?.replace(/^\/+/, "") ?? ""}`);
...
export default function (pi: ExtensionAPI) {
  ...
  if (enabledMcp("ripgrep")) {
    pi.registerTool({
      ...
      parameters: Type.Object({
        ...
        glob: Type.Optional(Type.String({ description: "Optional glob filter like '*.ts'" })),
        maxResults: Type.Optional(Type.Number({ description: "Optional per-file match cap. Omit for no rg cap." })),
      }),
      async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
        ...
        const text = `${result.stdout ?? ""}${result.stderr ?? ""}`.trim() || "No matches.";
        return {
  ...
  if (enabledMcp("searxng") || enabledMcp("brave-search")) {
    pi.registerTool({
      ...
      promptSnippet: "Use web_search for current web facts when local repo context is not enough.",
      promptGuidelines: [
      ...
      parameters: Type.Object({
        ...
        limit: Type.Optional(Type.Number({ description: "Requested result count", default: 10 })),
      }),
      async execute(_toolCallId, params) {
        ...
        const text = results.length === 0
          ...
          : results.map((r, i) => {
              const title = r.title ?? "";
              const url = r.url ?? "";
              const snippet = (r as { content?: string; description?: string }).content
                ?? (r as { content?: string; description?: string }).description
                ?? "";
              return `[${i + 1}] ${title}\n${url}${snippet ? `\n${snippet}` : ""}`;
            }).join("\n\n");
        return {
    ...
    pi.registerTool({
      ...
      promptSnippet: "Use web_fetch after web_search when you need page contents, not only search snippets.",
      parameters: Type.Object({
      ...
      async execute(_toolCallId, params) {
        ...
        return {
          content: [{ type: "text", text: body.slice(0, 20000) || "Empty response." }],
  ...
  if (enabledMcp("github")) {
    pi.registerTool({
      ...
      async execute(_toolCallId, params) {
        const results = await githubSearchRepositories(params.query, params.limit ?? 5);
        const text = results.length === 0
          ? "No results."
          : results.map((r, i) => {
    ...
    pi.registerTool({
      ...
      async execute(_toolCallId, params) {
        ...
        const text = results.length === 0
          ? "No results."
          : results.map((r, i) => `[${i + 1}] ${r.repository?.full_name ?? ""}/${r.path ?? ""}\n${r.html_url ?? ""}`).join("\n\n");
        return {
          content: [{ type: "text", text }],
          details: results,
        };
    ...
    pi.registerTool({
      ...
      parameters: Type.Object({
        ...
        ref: Type.Optional(Type.String({ description: "Branch, tag, or commit" })),
      }),
      async execute(_toolCallId, params) {
        const content = await githubReadFile(params.owner, params.repo, params.path, params.ref);
        return {
    ...
    pi.registerTool({
      ...
      parameters: Type.Object({
        owner: Type.String({ description: "Repository owner" }),
        repo: Type.String({ description: "Repository name" }),
        path: Type.Optional(Type.String({ description: "Directory path inside repository", default: "" })),
        ref: Type.Optional(Type.String({ description: "Branch, tag, or commit" })),
      }),
      async execute(_toolCallId, params) {
        const entries = await githubListDirectory(params.owner, params.repo, params.path ?? "", params.ref);
        const text = entries.length === 0
          ? "Empty directory."
          : entries.map((e) => `${e.type === "dir" ? "d" : "f"}  ${e.path ?? e.name ?? ""}  ${e.type !== "dir" ? `(${e.size ?? 0}b)` : ""}`).join("\n");
        return {
          content: [{ type: "text", text }],
          details: entries,
        };
  ...
  if (enabledMcp("npm-search")) {
    ...
    pi.registerTool({
      ...
      parameters: Type.Object({
        ...
        version: Type.Optional(Type.String({ description: "Version or dist-tag, e.g. 'latest', '1.0.0'. Defaults to latest." })),
      }),
      async execute(_toolCallId, params) {
        const encoded = encodeURIComponent(params.packageName);
        const ver = encodeURIComponent(params.version ?? "latest");
        const payload = await fetchJson(`https://registry.npmjs.org/${encoded}/${ver}`);
        return {
  ...
  if (enabledMcp("pypi")) {
    pi.registerTool({
      ...
      async execute(_toolCallId, params) {
        const payload = await fetchJson(`https://pypi.org/pypi/${encodeURIComponent(params.packageName)}/json`) as {
          info?: { name?: string; version?: string; summary?: string; home_page?: string; project_urls?: Record<string, string> };
          releases?: Record<string, unknown>;
        };
        const info = payload.info ?? {};
        const recentVersions = Object.keys(payload.releases ?? {}).slice(-5).join(", ");
        const text = [
  ...
  if (enabledMcp("rust-docs")) {
    ...
    pi.registerTool({
      ...
      async execute(_toolCallId, params) {
        const payload = (await cratesPackageInfo(params.crateName)) as {
          crate?: { name?: string; newest_version?: string; description?: string; repository?: string; documentation?: string; downloads?: number };
          versions?: Array<{ num?: string; created_at?: string }>;
        };
        const c = payload.crate ?? {};
        const versions = (payload.versions ?? []).slice(0, 5).map((v) => v.num ?? "").join(", ");
        const text = [
          `${c.name ?? ""}@${c.newest_version ?? ""}`,
          c.description ?? "",
          c.repository ? `repo: ${c.repository}` : "",
          c.documentation ? `docs: ${c.documentation}` : `docs: https://docs.rs/${c.name ?? ""}`,
    ...
    pi.registerTool({
      ...
      parameters: Type.Object({
        crateName: Type.String({ description: "Exact crate name" }),
        version: Type.Optional(Type.String({ description: "Crate version. Defaults to newest_version." })),
      }),
      async execute(_toolCallId, params) {
        const payload = await cratesDependencies(params.crateName, params.version);
        return {
          content: [{ type: "text", text: JSON.stringify(payload, null, 2) }],
          details: payload,
        };
    ...
    pi.registerTool({
      ...
      parameters: Type.Object({
        crateName: Type.String({ description: "Exact crate name" }),
        version: Type.Optional(Type.String({ description: "Crate version. Defaults to latest." })),
        path: Type.Optional(Type.String({ description: "Docs path under docs.rs, like tokio/fs/" })),
      }),
      async execute(_toolCallId, params) {
        const payload = await docsRsRead(params.crateName, params.version, params.path);
        return {
          content: [{ type: "text", text: payload.text.slice(0, 20000) || "Empty response." }],
          details: payload,
        };
      },
    });
