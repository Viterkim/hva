...
STATE_DIR="${HVA_LOOP_STATE_DIR:-/workspace/.hva-state/loop}"
...
first_iteration=1
initial_resume="${NANOCODER_AUTO_RESUME:-}"
...
run_prompt() {
  ...
  if (( first_iteration == 1 )); then
    if [[ "${HVA_LOOP_NEW_SESSION:-0}" == "1" ]]; then
      resume_value=""
    elif [[ -n "$initial_resume" ]]; then
      resume_value="$initial_resume"
    else
      resume_value="last"
    fi
  else
  ...
  if [[ -n "$resume_value" ]]; then
    NANOCODER_AUTO_RESUME="$resume_value" /usr/local/bin/nanocoder-real --mode yolo run "$prompt"
  else
    /usr/local/bin/nanocoder-real --mode yolo run "$prompt"
  fi

  first_iteration=0
  /hva/internals/save-session.sh || true
}
...
while true; do
  ...
  if run_prompt "$prompt"; then
    ...
    write_status "running" "$phase" "$current_target" "nanocoder exited with an error; continuing"
